...
import { fetchProjectTaskDetails, updateProjectStatus, updateStatus, updateTaskStatus } from "@/app/store/projectSlice";
import { useRouter } from "next/navigation";
import Swal from "sweetalert2";
import { useDispatch, useSelector } from "react-redux";

const Drawer01 = ({ isOpen, setIsDrawerOpen, details, itemId, setSelectedStatus }) => {
...
export const Drawer001 = ({ isOpen, setIsDrawerOpen, itemId, details }) => {
    ...
    const handleToggleStatus = async (event) => {
        ...
        if (result.isConfirmed && itemId) {
            ...
            // Dispatch updateUserStatus with the new status
            dispatch(updateTaskStatus({ id: itemId, project_status: newStatus, router }));
        }
    };
    if (!isOpen) return null;
    return (
                ...
                <div className="mb-4 mt-4 ml-[5px]">
                    ...
                    <ul className=" h-[22px] mt-[20px] ">
                        ...
                        <li className="flex mb-2 gap-4">
                            ...
                            <span className="text-gray-700 w-[200px] text-[14px]">{details?.task_type || ""}</span>
                        </li>
                        <li className="flex mb-2 gap-4">
                            <span className="text-gray-400 w-[120px] text-[14px]">Team</span><h4>:</h4>
                            <span className="text-gray-700 w-[200px] text-[14px]">{details?.team || ""} </span>
                        </li>
                        <li className="flex mb-2 gap-4">
                            <span className="text-gray-400 w-[120px] text-[14px]">Visibility</span><h4>:</h4>
                            <span className="text-gray-700 w-[200px] text-[14px]">{details?.visibility || ""}</span>
                        </li>
                        <li className="flex mb-2 gap-4">
                            <span className="text-gray-400 w-[120px] text-[14px]">Link</span><h4>:</h4>
                            <span className="text-gray-700 w-[200px] text-[14px]">{details?.link || ""}</span>
                        </li>
